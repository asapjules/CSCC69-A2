...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t *victim = coremap[frame].pte; // pointer to the victim page
		// update counter
		if((victim->frame & PG_DIRTY) == 0){ //if dirty bit is not set, then it is clean
			evict_clean_count++;
		}
		else{ // if dirty bit is set, then it is dirty
			// since the page has been modified, flag for swap and reset dirty bit
		...
		if(swap_offset == INVALID_SWAP){ //if the swap failed
			perror("unable to swap out");
			exit(EXIT_FAILURE);
		}
		else{
			victim->swap_off = swap_offset;
		}
		// page is swapped out; invalid
...
char *find_physpage(addr_t vaddr, char type) {
	...
	uintptr_t pde = (pgdir[idx].pde); // get page directory entry from pgd
	if((pde & PG_VALID) == 0){ // if pde not valid
		pgdir[idx] = init_second_level();
		pde = pgdir[idx].pde;
	}
	pgtbl_entry_t *pte = (pgtbl_entry_t*)(pde & PAGE_MASK); // pointer to 2nd-level page table
	...
	if(p->frame & PG_VALID){ //if p is valid
		hit_count++;
	}
	else{ //if p not valid
		int frame = allocate_frame(p); //allocate a physical frame for p
		if(p->frame & PG_ONSWAP){ //if p on swap
			if(swap_pagein(frame, p->swap_off)){ //if data is read into physical memory unsuccessfully
				perror("unable to swap in");
				exit(EXIT_FAILURE);
			}
			p->frame = frame << PAGE_SHIFT;
			p->frame = p->frame | PG_ONSWAP; //on swap
			p->frame = p->frame & ~PG_DIRTY; //newly filled; not modified
		}
		else{ //if p not on swap
			//initialize frame for p
			init_frame(frame, vaddr);
			p->frame = frame << PAGE_SHIFT;
			p->frame = p->frame | PG_DIRTY; //newly initialized
		}
		miss_count++;
	}
